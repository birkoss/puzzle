...
export class DungeonScene extends Phaser.Scene {
    ...
    #createTiles(width, height) {
        ...
        const x = (this.scale.width - (width * TILE_SIZE)) / 2;
        const y = (this.scale.height - 220 - 100 - (height * TILE_SIZE)) / 2 + 100;

        for (let y2 = 0; y2 < height+2; y2++) {
            for(let x2 = 0; x2 < width+2; x2++) {
                let frame = Phaser.Math.Between(1, 100) <= 50 ? 85 : 131;
                if ((y2 === 0 && x2 > 0 && x2 < width+1) || y2 === height+1) {
                    frame = Phaser.Math.Between(1, 100) <= 50 ? 62 : 108;
                }

                let background = this.add.sprite(TILE_SIZE * x2 + TILE_SIZE/2 + x, TILE_SIZE * y2 + TILE_SIZE/2 + y, MAP_ASSET_KEYS.WORLD, frame).setScale(3);
        ...
        for (let y = 0; y < height; y++) {
            for(let x = 0; x < width; x++) {
                let frame = ((y * height) + x) % 2 === 0 ? 94 : 99;
                let background = this.add.sprite(TILE_SIZE * x + TILE_SIZE/2, TILE_SIZE * y + TILE_SIZE/2, MAP_ASSET_KEYS.WORLD, frame).setScale(3);
        ...
        for (let y = 0; y < height; y++) {
            for(let x = 0; x < width; x++) {
                ...
                let tile = new Tile(x, y, new Block(container, null, icon));
